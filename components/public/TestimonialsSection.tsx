'use client';

import React from 'react';
import { useDataContext } from '@/lib/data-context';
import { getTranslation } from '@/lib/dictionary';
import { Star, MessageSquare } from 'lucide-react';

export const TestimonialsSection: React.FC = () => {
  const { testimonials, language } = useDataContext();

  const approvedTestimonials = testimonials.filter(t => t.isApproved);

  return (
    <section id="testimonials" className="py-20 bg-[#050505] text-[#F5F5F5] border-b border-white/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-3">
          <div className="inline-flex items-center space-x-2 px-3 py-1 bg-[#0F0F0F] border border-white/10 rounded-sm text-[10px] uppercase tracking-[0.25em] text-white/60 font-bold">
            <MessageSquare className="w-3.5 h-3.5 text-white/80" />
            <span>Endorsements & Reviews</span>
          </div>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-serif italic text-white tracking-tight">
            {getTranslation(language, 'testimonials.title')}
          </h2>
          <p className="text-xs sm:text-sm text-white/50 uppercase tracking-widest font-mono">
            {getTranslation(language, 'testimonials.subtitle')}
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {approvedTestimonials.map((item) => (
            <div
              key={item.id}
              className="p-8 bg-[#0F0F0F] border border-white/10 rounded-sm space-y-4 flex flex-col justify-between hover:border-white/30 transition shadow-xl"
            >
              <div className="space-y-3">
                {/* Rating Stars */}
                <div className="flex items-center space-x-1 text-white">
                  {Array.from({ length: item.rating }).map((_, idx) => (
                    <Star key={idx} className="w-3.5 h-3.5 fill-white text-white" />
                  ))}
                </div>

                <p className="text-sm text-white/80 font-serif italic leading-relaxed">
                  &ldquo;{item.content[language]}&rdquo;
                </p>
              </div>

              <div className="flex items-center space-x-3 pt-4 border-t border-white/10">
                <img
                  src={item.avatarUrl}
                  alt={item.clientName}
                  className="w-10 h-10 rounded-sm object-cover border border-white/10 bg-[#1A1A1A]"
                />
                <div>
                  <h4 className="font-serif italic text-white text-base">
                    {item.clientName}
                  </h4>
                  <p className="text-[10px] uppercase tracking-wider text-white/50 font-mono">
                    {item.titleRole} — <span className="text-white font-semibold">{item.companyName}</span>
                  </p>
                </div>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
};
